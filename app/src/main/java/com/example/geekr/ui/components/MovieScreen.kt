...
fun MovieScreen(navController: NavHostController){
    ...
    if(user != null){
        MovieScreenWithCompactMenu(navController) {
...
@Composable
fun MovieScreenWithCompactMenu(navController: NavHostController, content: @Composable () -> Unit) {
        ...
        Scaffold(
            topBar = {
                TopAppBar(
                    ...
                    title = { Text(text = "Geekr en 35mm", fontSize = MaterialTheme.typography.headlineMedium.fontSize, color = Color.White) },
                    navigationIcon = {
...
@Composable
fun MovieSearchScreen(navController: NavHostController, viewModel: MovieViewModel = viewModel()) {
    ...
    Column(
            ...
            BasicTextField(
                ...
                decorationBox = { innerTextField ->
                    if (searchQuery.text.isEmpty()) {
                        Text("Escribe el nombre de una película...", color = Color.LightGray)
                    }
                    innerTextField()
                },
        ...
        Button(
            onClick = { viewModel.searchMovie(BuildConfig.API_KEY, searchQuery.text) },
            modifier = Modifier.fillMaxWidth(),
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            items(movieList) { movie ->
                ...
                Column(
                    ...
                    Text("⭐ Calificación: ${movie.vote_average}", color = Color.White)
                    Spacer(modifier = Modifier.height(4.dp))
                    Text("📅 Estreno: ${auxFunctions.formatDate(movie.release_date)}", color = Color.White)
